  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    let ls = client.settings.get(message.guild.id, "language")
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addField('<:a_right:933701797032361984> **Name**', "```"+json.name+"```", true)
        .addField("<:a_right:933701797032361984> **Hex**",  "```"+json.hex+"```", true)
        .addField("<:a_right:933701797032361984> **RGB**",  "```"+json.rgb+"```", true)
        .addField(`<:a_right:933701797032361984> **${client.la[ls].cmds.info.color.brightershade}**`,  "```"+json.brightened +"```", true)
        .setThumbnail(json.color_image)
        .setColor(json.hex)
      if(userinfo) embed.addField("<a:Valid_Code_Developer:933742305398116363> Color ==  Your Highest Role!", `> Usage: \`${prefix}color ${args[0]}\``);
      message.reply({
        embeds: [embed]
      })
    } catch (e) {
