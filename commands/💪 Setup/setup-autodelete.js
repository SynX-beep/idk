    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor('Auto Delete Setup', 'https://cdn.discordapp.com/emojis/834052497492410388.gif?size=96', 'https://discord.gg/Azury')
                  ...
                  try {
                    ...
                    if(a.map(d => d.id).includes(channel.id))
                      return message.reply({embeds: [new Discord.MessageEmbed()
                        .setTitle(`${client.emoji.no} This Channel is already Setupped!`)
                    ...
                    if(!time || isNaN(time))
                      return message.reply({embeds: [new Discord.MessageEmbed()
                        .setTitle(`${client.emoji.no} Invalid Input | Time wrong`)
                    ...
                    if(time > 60*60 || time < 3)
                      return message.reply({embeds: [new Discord.MessageEmbed()
                        .setTitle(`${client.emoji.no} Time out of Range!`)
                        .setDescription(`The longest Amount is 1 hour aka 3600 Seconds and the Time must be at least 3 Seconds long!`)
                        .setColor(es.color)
                        .setFooter(es.footertext, es.footericon)
                      ]}); // Can u do the setup cmds? Ill do the en.json Language file fine
                    client.setups.push(message.guild.id, { id: channel.id, delay: time * 1000 }, "autodelete")
                    return message.reply({embeds: [new Discord.MessageEmbed()
                      .setTitle(`${client.emoji.yes} I will now delete Messages after \`${time} Seconds\` in **${channel.name}**`)
                      .setColor(es.color)
                      .setFooter(es.footertext, es.footericon)
                    ]});
                  } catch (e) {
              ...
              .then(collected => {
                ...
                if (channel) {
                  try {
                    ...
                    if(!a.map(d => d.id).includes(channel.id))
                    return message.reply({embeds: [new Discord.MessageEmbed()
                      .setTitle(`${client.emoji.no} This Channel has not been Setup yet!`)
                      .setColor(es.color)
                      .setFooter(es.footertext, es.footericon)
                    ]});
                    client.setups.remove(message.guild.id, d => d.id == channel.id, "autodelete")
                    return message.reply({embeds: [new Discord.MessageEmbed()
                      .setTitle(`${client.emoji.yes} Successfully removed **${channel.name}** out of the Setup!`)
                      .setColor(es.color)
                      .setFooter(es.footertext, es.footericon)
                    ]});
                  } catch (e) {
