      ...
      async function first_layer() {
        ...
        collector.on('collect', menu => {
          if (menu?.user.id === cmduser.id) {
            ...
            content: `${client.emoji.no} You are not allowed to do that! Only: <@${cmduser.id}>`,
            ephemeral: true
          });
        });
        ...
        collector.on('end', collected => {
          menumsg.edit({
            ...
            components: [],
            content: `${client.emoji.yes} **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**`
          })
        });
      }
        ...
        switch (optionhandletype) {
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply(""+client.emoji.no+" **You need to add at least 1 Auto-Support-Option**")
            }
            ...
            if (collected && collected.first().content) {
              ...
              if (collected2 && collected2.first().mentions.channels.size > 0) {
                ...
                return message.reply(`${client.emoji.no} **You did not ping a valid Channel!**`)
              }
            } else {
              return message.reply(`${client.emoji.no} **You did not enter a Valid Message in Time! CANCELLED!**`)
            }
          }
          break;
          case "Add AutoSup Option": {
            let data = client.autosupport.get(message.guild.id, "data");
            if (data.length >= 25) {
              return message.reply(`${client.emoji.no} **You reached the limit of 25 different Options!** Remove another Option first!`)
            }
            ...
            if (collected && collected.first().content) {
              ...
              if(index2 >= 0 && index != index2) {
                  return message.reply("${client.emoji.no} **Options can't have the SAME VALUE!** There is already an Option with that Value!");
              }
            ...
            collector.on('end', collected => {
              tempmsg.edit({
                ...
                components: [],
                content: `${client.emoji.yes} **Selected: \`${collected ? collected.customId : "Nothing | CANCELLED"}\`**`
              })
            });
            } else {
              return message.reply(`${client.emoji.no} **You did not enter a Valid Message in Time! CANCELLED!**`)
          ...
          case "Edit AutoSup Option": {
            let data = client.autosupport.get(message.guild.id, "data");
            if (!data || data.length < 1) {
              return message.reply(`${client.emoji.no} **There are no Open-Ticket-Options to edit**`)
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon : null)
            ...
            collector.on('collect', async menu => {
              if (menu?.user.id === cmduser.id) {
                ...
                if (collected && collected.first().content) {
                  if (!collected.first().content.includes("++")) return message.reply(`${client.emoji.no} **Invalid Usage! Please mind the Usage and check the Example**`)
                  let value = collected.first().content.split("++")[0].trim().substr(0, 25);
                  let index2 = data.findIndex(v => v.value == value);
                  if(index2 >= 0 && index != index2) {
                      return message.reply(`${client.emoji.no} **Options can't have the SAME VALUE!** There is already an Option with that Value!`);
          ...
          let data = client.autosupport.get(message.guild.id, "data");
          if (!data || data.length < 1) {
            return message.reply(`${client.emoji.no} **There are no Auto-Responding-Support-Options to remove**`)
          }
          let embed = new MessageEmbed()
            .setColor(es.color)
            .setThumbnail(es.thumb ? es.footericon : null)
