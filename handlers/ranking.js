     ...
     try{
        ...
        async function LEVELUP() {
          let rankuser = message.author;
            ...
            await canvacord.Util.renderEmoji(ctx, `LEVEL: ${newLevel}`, 475, 290);
            //AVATAR
            ctx.beginPath();
           ...
           if(!client.points.get(message.guild.id, "channel")) return message.channel.send({content: ` ${message.author} **YOU LEVELED UP! 🎉**`, files: [attachment]});
            try{
                let channel = message.guild.channels.cache.get(client.points.get(message.guild.id, "channel"))
                if(!channel){
                    return message.channel.send({content: `${message.author} **YOU LEVELED UP! 🎉**`, files: [attachment]}).catch(() => {})
                }
                channel.send({content: `${message.author} **YOU LEVELED UP! 🎉**`, files: [attachment]});

            }catch (e){
                message.channel.send({content: `${message.author} **YOU LEVELED UP! 🎉**`, files: [attachment]}).catch(() => {})
            }
        }
        ...
        async function rank(the_rankuser, type = "text") {
            ...
            try {
                ...
                let tempmessage = await message.channel.send(`<a:Loading:920516789883002880> *Getting the ${type == "voice" ? "🔉" : "💬"}__${type.toUpperCase()}__-RANK of: **${rankuser.tag}** ...*`)
                //do some databasing
                const filtered = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_Tomato_6966 => this_Code_is_by_Tomato_6966);
                const sorted = filtered
                .sort((a, b) => { 
        ...
        async function newleaderboard(type = "text") {
            let theDbDatas = ["level", "points", "neededpoints", ]
            if(type == "voice") theDbDatas = ["voicelevel", "voicepoints", "neededvoicepoints", ]
            let tempmessage = await message.channel.send(`📊 *Getting the ${type == "voice" ? "🔉" : "💬"}__${type.toUpperCase()}__-LEADERBOARD of: **${message.guild.name}** ...*`)
            ...
            new Promise(async (res, rej)=>{
                ...
                new Promise(async (res, rej)=>{
                    ...
                    message.channel.send({content:`Top 10 Leaderboard of **${message.guild.name}** Sorted after ${type.toUpperCase()}-POINTS\n> **Type:** \`leaderboard all\` to see all Ranks\n*Rank is counted for the \`${type.toUpperCase()}-RANK\`*\n> ${type != "voice" ? `To see the **Voice Leaderboard** type: \`voiceleaderbaord [all]\`` : `To see the **Text Leaderboard** type: \`leaderbaord [all]\``}`, files: [attachment]}).catch(() => {});
