...
module.exports = (client) => {
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    guild.channels.create(channelname.substr(0, 31), optionsData).then(async ch => {
      ...
      if(ticket.claim.enabled){
        allbuttons.push(new MessageActionRow().addComponents([new MessageButton().setStyle('SECONDARY').setCustomId('ticket_claim').setLabel("Claim the Ticket").setEmoji(client.emoji.yes)]))
      }
