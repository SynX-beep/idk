...
module.exports = (c) => {
    ...
    c.on("messageDelete", async function (message) {
        send_log(c,
          ...
          "ORANGE",
          "Message Deleted", 
          `**Author : ** <@${message.author?.id}> - *${message.author?.tag}*\n**Date : ** ${message.createdAt}\n**Channel : ** <#${message.channel?.id}> - *${message.channel?.name}*\n\n**Deleted Message : **\n\`\`\`\n${message.content?.replace(/`/g, "'").substr(0, 1800) || `Embed or Unknown Content`}\n\`\`\``,
          "https://cdn.discordapp.com/attachments/849047781276647425/869530655871082516/850923749132992550.png",
...
async function send_log(c, guild, color, title, description, thumb, fieldt, fieldv, fieldt2, fieldv2) {
  try {
    ...
    const LogEmbed = new Discord.MessageEmbed()
      ...
      .setFooter(guild?.name + " ⚡ discord.gg/notsaksh • .gg/azury", guild.iconURL({ //me server? :( :cry: :O ayo ty
        format: "png"
      })) // np, u can add ur credits anywhere u want
    if(fieldt && fieldv){
